...
COMMON_PASSWORDS = {
    ...
    'bolbok', 'bolbok123', 'batangas', 'batangas123',
    'barangaybolbok', 'bolbokbatangas',
}
...
# ── Fixed coordinates — Barangay Bolbok, Batangas City, Batangas ──────────────
TIDE_LAT = 13.7565
TIDE_LON  = 121.0583
...
def _ensure_initial_user():
    ...
    try:
        ...
        print(f'[BBCB] WARNING: Could not check users table: {exc}')
        return
    ...
    if not plain_pw:
        ...
        print('[BBCB] -------------------------------------------------')
        print('[BBCB] WARNING: No ADMIN_PASSWORD set.')
        print(f'[BBCB] Auto-generated password: {plain_pw}')
        print('[BBCB] Log in and change your password immediately.')
    else:
        print('[BBCB] -------------------------------------------------')
        print('[BBCB] Admin account created from ADMIN_PASSWORD env var.')
        print(f'[BBCB] Username : {username}')
        print('[BBCB] Log in and change your password immediately.')
    ...
    try:
        ...
        print(f'[BBCB] Initial admin user created: {username}')
    except Exception as exc:
...
def _load_officials():
    try:
        ...
        print(f'[BBCB] ERROR loading officials: {exc}')
        return []
...
def api_weather():
    ...
    try:
        url = 'https://wttr.in/Batangas+City,Batangas?format=j1'
...
def api_tide():
    ...
    try:
        ...
        req = urllib.request.Request(url, headers={'User-Agent': 'BBCB-Site/1.0'})
        with urllib.request.urlopen(req, timeout=10) as resp:
            import json as _json
            raw = _json.loads(resp.read().decode('utf-8'))
...
# ── Public page routes — clean URLs and .html extensions both supported ───────
_PUBLIC_PAGES = [
...
def admin_proj_create():
    ...
    proj = {
        'id':               uuid.uuid4().hex,
        'title':            _clean(d.get('title'), 200),
        'category':         _clean(d.get('category', 'General'), 50),
        'subtitle':         _clean(d.get('subtitle'), 200),
        'shortDescription': _clean(d.get('shortDescription'), 500),
        'fullDetails':      _clean(d.get('fullDetails'), 10000),
        'imageUrl':         _clean(d.get('imageUrl'), 300),
        'status':           status,
...
def admin_proj_update(proj_id):
    d = request.get_json(silent=True) or {}
    patch = {}
    for field, maxlen in [('title', 200), ('category', 50), ('subtitle', 200),
...
def admin_forms_create():
    ...
    form = {
        ...
        'fileType':      _clean(d.get('fileType', 'pdf'), 20),
...
def admin_forms_update(form_id):
    d = request.get_json(silent=True) or {}
    patch = {}
    for field, maxlen in [('title', 200), ('description', 500),
                          ('fileUrl', 300), ('fileName', 200), ('fileType', 20)]:
        if field in d:
            patch[field] = _clean(d[field], maxlen)
    if 'fileSize' in d:
...
def admin_hotlines_create():
    d = request.get_json(silent=True) or {}
    label    = _clean(d.get('label'), 200)
    number   = _clean(d.get('number'), 100)
    if not label or not number:
        return jsonify({'error': 'Label and number are required.'}), 400
    now = datetime.now(timezone.utc).isoformat()
    row = {
        'label':         label,
        'number':        number,
        ...
        'status':        'published',
        'display_order': int(d.get('displayOrder', 99)),
        'created_at':    now,
        'updated_at':    now,
    }
    res = supabase.table('emergency_hotlines').insert(row).execute()
    hotline = _row_to_hotline(res.data[0]) if res.data else row
    return jsonify({'ok': True, 'hotline': hotline}), 201
...
def admin_hotlines_update(hotline_id):
    ...
    if 'label' in d:
        patch['label'] = _clean(d['label'], 200)
    if 'number' in d:
        patch['number'] = _clean(d['number'], 100)
    if 'description' in d:
        patch['description'] = _clean(d['description'], 300)
    if 'category' in d:
        patch['category'] = _clean(d['category'], 100)
    if 'status' in d and d['status'] in ('published', 'hidden'):
        patch['status'] = d['status']
    if 'displayOrder' in d:
...
# ── Directory Module (Phase 2) ────────────────────────────────────────────────
# Community Map / Business Directory / Organization Directory / Emergency
# Directory — CRUD-managed replacement for the Phase 1 hardcoded sample data.
# Mirrors the existing community-initiatives / officials / forms pattern
# exactly: _row_to_X converters, _X_create/_update/_delete helpers, a public
